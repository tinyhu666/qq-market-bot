...
issue005 [x] 标题:AI 与海外科技新闻摘要出现泛化和无主体短句 | 模块:scripts/新闻摘要 | 优先级:P1 | 关联:task053 | 解决:2026-05-08

## 现象

- 用户反馈海外科技 Top5 总结“几乎不可读”，AI Top10 中也有部分新闻语意不明。
- 真实 dry-run 中可复现海外科技摘要出现“亚马逊发布海外科技新进展”“Perplexity发布海外科技新进展”等空泛模板句。
- AI 区也会出现“支持代理交易。”这类没有主体、没有对象的短句，或把英文标题截断后直接进入最终消息。

## 复现

- 服务器 dry-run 输出 `海外科技 Top 5` 时，多个候选都被同一个“发布海外科技新进展”模板覆盖，读者无法知道实际事件。
- 本地关闭 LLM 后调用 `collectNews`，AI Top10 中会出现 `Parloa builds...`、`Model Quantization...`、`Claude's new "Dreaming"...` 等英文截断或不完整摘要。

## 根因

- 海外科技区此前只依赖固定规则模板和少量标题兜底，未接入 LLM 摘要路径，遇到未知公司/标题时容易退回“发布新进展”。
- AI 摘要质量门只过滤了部分营销腔和 clickbait，没有覆盖无主体谓语短句、中文上下文缺失句和当前真实源里的高频英文标题。
- AI 普通 item 构建路径没有充分复用 `buildHeuristicTechAiSummaryFromTitle`，导致即使已有兜底规则，也可能在非 LLM 路径继续输出英文标题截断。

## 修复

- 海外科技区新增 LLM 中文硬新闻摘要链路，要求按候选顺序输出主体、动作和对象明确的中文短句，并拒绝“发布海外科技新进展”等泛化句。
- AI/海外科技共用更严格的质量门，拦截“支持代理交易。”这类无主体短句、营销腔、英文截断和上下文缺失句。
- AI 普通 item 构建路径也优先使用规则化中文兜底，并为本轮真实样本补充 Parloa、GPT-5.5 Cyber、OpenAI 语音模型、NVIDIA 量化、Claude Dreaming、AWS AgentCore、Claude 桌面端限制、云知声和波士顿动力等明确中文摘要。
- 新增回归测试覆盖海外科技 LLM 泛化摘要回退、AI 无主体 LLM 摘要回退、英文 AI 标题中文化以及中文营销/上下文缺失摘要替换。
