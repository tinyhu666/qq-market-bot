...
- `OpenAI News`、`Google AI`、`NVIDIA`、`VentureBeat AI`、`量子位` 过去 `24` 小时内合并后的 AI `10` 条
- `follow-builders` 公开 feed 中筛出来的 `X/blog` 动态
...
- `AI` 新闻优先抓取 `OpenAI News`、`Google AI`、`NVIDIA Blog`、`NVIDIA Technical Blog`、`VentureBeat AI` 这类国际源
- 国内 AI 候选默认优先使用 `量子位`；`AIBase` 解析器暂时保留在代码里，但已从默认主榜候选源中移除，避免低质量聚合内容污染热榜
- `X/blog` 模块会额外抓取 [`follow-builders`](https://github.com/zarazhangrui/follow-builders) 公开 raw feed，优先展示其中事件性更强的 `X / blog` 动态
...
`X/blog` 模块不会并入 `AI Top 10`。它会单独从 `follow-builders` 的 `X / blogs` 公开 feed 里挑选更像“事件更新”的条目，过滤掉纯感想、宣传导流和低信号转评，再在可用时交给同一套 LLM 生成中文短句。这样可以把 builder 圈的一手动态展示出来，但不污染主 AI 热榜。

为了不挤压原有 `AI` 和 `财经` 区块，`X/blog` 在最终消息里最多只展示前 `5` 条；如果候选多于 `5` 条，会在区块底部补一行详情链接，优先跳到第一条未展示内容的真实详情页；只有拿不到详情 URL 时，才回退到仓库首页。
...
脚本现在只在“同一轮最终消息”里做去重：同一轮播报会做跨栏目去重，并且在每条最终要发出的消息组装完成前再做一次消息级去重，尽量避免同一事件在 `AI`、`X/blog`、`财经` 甚至同一条消息内重复出现。不同时间段之间不再做“当天已发过”的跨时段去重，避免上午出现的热点在中午或晚上被错误跳过。
...
- 新闻相关可选：
  ...
  - `MARKET_FOLLOW_BUILDER_NEWS_LIMIT`，默认 `5`；用于控制 `X/blog` 候选池大小，但最终消息仍最多只显示前 `5` 条
...
【X/blog Top 5】
1. ...
更多总结：点击查看详情 https://www.anthropic.com/engineering/infrastructure-noise
