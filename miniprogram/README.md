...
为适配新的国际源链路，脚本额外支持了解析 `Atom feed` 与 `HTML 列表页`：例如 `NVIDIA Technical Blog` 使用 `Atom`，`AIBase` 使用服务端渲染列表页解析。当前默认策略是“高质量国际/国内主源优先 + 低优先级补量源限流”，避免 AI 区长期缩到 `Top 3`，同时尽量不让聚合源主导最终热榜；对海外 RSS 里常见的 `&apos;`、数字实体等标题编码也会先解码再进入筛选，减少候选标题被截断或变形。
