...
- `OpenAI News`、`Google AI / DeepMind`、`AI HOT`、`NVIDIA`、`Microsoft Research`、`AWS Machine Learning`、`Hugging Face`、`MIT Technology Review AI`、`The Decoder`、`AI News`、`量子位` 等源过去 `24` 小时内合并后的 AI 热榜
...
- `AI` 新闻优先抓取 `OpenAI News`、`Google AI`、`Google DeepMind`、`AI HOT`、`NVIDIA Blog`、`NVIDIA Technical Blog`、`Microsoft Research`、`AWS Machine Learning Blog`、`Hugging Face Blog`、`VentureBeat AI`、`TechCrunch AI`、`MIT Technology Review AI`、`The Decoder`、`AI News` 这类国际源和高优先级精选源；`AI HOT` 走 `https://aihot.virxact.com/api/public/items?mode=selected` 匿名公开接口，并携带浏览器 UA 请求头
- `海外科技` 默认抓取 `The Verge`、`Ars Technica`、`Engadget`、`TechCrunch`、`MIT Technology Review` 等海外科技源，单独输出 `5` 条；英文 RSS 标题会保留作溯源和去重，但最终消息行会优先通过大模型改写成中文硬新闻摘要，并拒绝“发布海外科技新进展”这类泛化句；如果与前面的 `AI Top 10` 命中同一事件，会被同轮去重过滤
...
为适配新的国际源链路，脚本额外支持了解析 `Atom feed`、`HTML 列表页` 与 `AI HOT selected items JSON`：例如 `NVIDIA Technical Blog` 使用 `Atom`，`AIBase` 使用服务端渲染列表页解析，`AI HOT` 使用公开精选 API 解析。当前默认策略是“高质量国际/国内主源优先 + 高优先级精选源补强 + 低优先级补量源限流”，避免 AI 区长期缩到 `Top 3`，同时尽量不让聚合源主导最终热榜；对海外 RSS 里常见的 `&apos;`、数字实体等标题编码也会先解码再进入筛选，减少候选标题被截断或变形。
...
脚本现在只在“同一轮最终消息”里做去重：`AI`、`财经` 与 `海外科技` 会做跨栏目去重，并且在每条最终要发出的消息组装完成前再做一次消息级去重，尽量避免同一事件在这些栏目或同一条消息内重复出现。不同时间段之间不再做“当天已发过”的跨时段去重，避免上午出现的热点在中午或晚上被错误跳过。
...
- 新闻相关可选：
  ...
  - `MARKET_OVERSEAS_TECH_NEWS_LIMIT`，默认 `5`
