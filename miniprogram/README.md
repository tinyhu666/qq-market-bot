...
- 仅使用海外 AI 源单独生成的 `海外AI Top 5`，不参与挤占原 `AI Top 10` 与财经区
...
- `海外AI` 会复用上述国际源并单独输出默认 `5` 条，作为独立观察榜；它只做段内去重，不会影响原来的 `AI Top 10` 与财经区去重结果
...
脚本现在只在“同一轮最终消息”里做去重：`AI` 与 `财经` 会做跨栏目去重，并且在每条最终要发出的消息组装完成前再做一次消息级去重，尽量避免同一事件在这两栏或同一条消息内重复出现。`海外AI` 是独立观察榜，只做本段内去重，允许和主 `AI` 榜展示同一海外热点。不同时间段之间不再做“当天已发过”的跨时段去重，避免上午出现的热点在中午或晚上被错误跳过。
...
- 新闻相关可选：
  ...
  - `MARKET_OVERSEAS_AI_NEWS_LIMIT`，默认 `5`
